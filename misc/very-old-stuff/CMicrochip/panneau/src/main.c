...
#define RelacheBPs() while (!BPcentre | !BPgauche | !BPdroit)
...
Uchar Quit = 0;
Uchar Menu[][SIZE_STR_LCD] = {"<-    AUTO    ->",
                              "<-    MANU    ->",
                              "<-  CONSIGNE  ->",
                              "<-   HEURE    ->",
                              "<-  REGLAGES  ->"
                             };
Uchar ModeSTR[][SIZE_STR_LCD]    = {"MODE AUTOMATIQUE",
                                    "<- G        D ->",
                                    "     GAUCHE     ",
                                    "     DROITE     "
                                   };
...
/* IHM
    ...
    Code utilis� pour entre dans un mode de la
    tache de fond.
...
void DisplayMenu(Uchar from, Uchar to, Uchar *IHM_code);
...
void ModeAuto(void)
{
    ...
    PAUSE_x10ms(25);  //Tempo a mettre pour les tests pas sur le materiel
}
...
void ModeManu(void)
{
    ...
    {
        MessageLCDram(ModeSTR[2]);
        RelaisV = 0;
        RelaisR = 1;
    }
    else if(!BPdroit & BPcentre)
    {
        MessageLCDram(ModeSTR[3]);
        RelaisV = 0;
        RelaisR = 0;
    }
    else
...
void DisplayMenu(Uchar from, Uchar to, Uchar *IHM_code)
{
    ...
    if(to < i || i < from)  //Si l'indice d'IHM est en dehors des parametres
    {
        i = from;
    }
